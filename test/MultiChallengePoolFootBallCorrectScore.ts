...
async function deployChallengePool() {
  ...
  const MultiFootBallCorrectScoreEvaluator = await ethers.getContractFactory(
    "MultiFootBallCorrectScoreEvaluator"
  );

  const footBallCorrectScoreEvaluator =
    await MultiFootBallCorrectScoreEvaluator.deploy(
...
async function deployCreateChallenges() {
  ...
  const result = coder.encode(["uint256", "uint256"], [BigInt(7), BigInt(9)]);
  const homeScore = BigInt(7);
  const awayScore = BigInt(9);
  ...
  describe("Create Challenge", function () {
    ...
    it("Should Fail to Create Challenge", async function () {
      ...
      const invalidPollOption = [
        ...
        coder.encode(["uint256", "uint256"], [BigInt(9), BigInt(10)]),
        ethers.toUtf8Bytes(""),
      ];
      await expect(
        pool
      ...
      ).to.revertedWithCustomError(pool, "InvalidPollOption");
    });
  });
