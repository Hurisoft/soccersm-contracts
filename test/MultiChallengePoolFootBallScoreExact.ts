...
async function deployChallengePool() {
  ...
  const MultiFootBallScoreExactEvaluator = await ethers.getContractFactory(
    "MultiFootBallScoreExactEvaluator"
  );

  const footBallScoreExactEvaluator =
    await MultiFootBallScoreExactEvaluator.deploy(
      await footBallScoreProvider.getAddress()
    );

  await footBallScoreProvider.addReader(
    await footBallScoreExactEvaluator.getAddress()
  );
...
async function deployCreateChallenges() {
  ...
  const options = [BigInt(2), BigInt(4), BigInt(6), BigInt(9), BigInt(10)].map(
    (o) => coder.encode(["uint256"], [o])
  );
  const param1 = coder.encode(["uint256"], [matchId]);
  const result = coder.encode(["uint256"], [BigInt(9)]);
  const homeScore = BigInt(1);
  const awayScore = BigInt(8);
  ...
  const userPrediction = coder.encode(["uint256"], [BigInt(9)]);
...
describe("MultiChallengePoolFootBallScoreRange", function () {
  ...
  describe("Create Challenge", function () {
    ...
    it("Should Fail to Create Challenge", async function () {
      ...
      const InvalidOptionsLength2 = [
        coder.encode(["uint256"], [BigInt(2)]),
      ];
      const invalidPollOption = [
        coder.encode(["uint256"], [BigInt(2)]),
        coder.encode(["uint256"], [BigInt(9)]),
        ethers.toUtf8Bytes(""),
      ];
  ...
  describe("Join Challenge", function () {
    it("Should Join Challenge", async function () {
      ...
      const kofiPrediction = coder.encode(["uint256"], [BigInt(2)]);

      const amaPrediction = coder.encode(["uint256"], [BigInt(4)]);
    ...
    it("Should Fail to Join Challenge", async function () {
      const { yaa, abena, ballsToken, pool, airDropBalls, joinStakeFee } =
        await loadFixture(deployCreateChallenges);
      const coder = new ethers.AbiCoder();
      const abenaPrediction = coder.encode(["uint256"], [BigInt(2)]);
  ...
  describe("Close Challenge", function () {
    it("Should Close Challenge", async function () {
      ...
      const coder = new ethers.AbiCoder();
      const adwoaPrediction = coder.encode(["uint256"], [BigInt(2)]);
  ...
  describe("Withdraw Winnings", function () {
    it("Should Withdraw Winnings", async function () {
      ...
      const coder = new ethers.AbiCoder();
      const kwamePrediction = coder.encode(["uint256"], [BigInt(4)]);
