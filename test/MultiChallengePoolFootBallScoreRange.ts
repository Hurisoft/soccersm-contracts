...
async function deployChallengePool() {
  ...
  const MultiFootBallScoreRangeEvaluator = await ethers.getContractFactory(
    "MultiFootBallScoreRangeEvaluator"
  );

  const footBallScoreRangeEvaluator =
    await MultiFootBallScoreRangeEvaluator.deploy(
      await footBallScoreProvider.getAddress()
    );

  await footBallScoreProvider.addReader(
    await footBallScoreRangeEvaluator.getAddress()
  );

  await footBallScoreProvider.addProvider(await kofi.getAddress());

  await registry.createTopic(
    "FootBall Score Range",
    "FootBall Score Range",
    await footBallScoreRangeEvaluator.getAddress()
  ); // 0
